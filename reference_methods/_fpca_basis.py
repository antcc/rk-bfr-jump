...
class KarhunenLoeve(Basis):
    ...
    def _compute_fpcs(self):
        ...
        if isinstance(self.X, FDataGrid):
            ...
            fpca_components = fpca_components.reshape(fpca_components.shape[:-1])
    ...
    def _evaluate(self, eval_points: np.ndarray) -> np.ndarray:
        ...
        if isinstance(self.X, FDataGrid):
            eval_points_idx = np.abs(
                self.X.grid_points[0] - eval_points[:, np.newaxis]
            ).argmin(1)
...
class FPCABasis(
    ...
    def transform(self, X: FData, y: None = None) -> FDataBasis:
        if X.equals(self.X):
            return FDataBasis(self.expansion, self.expansion.coefficients)
