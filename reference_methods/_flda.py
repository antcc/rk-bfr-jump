...
class FLDA(
    ...
    def fit(  # noqa: D102
        ...
        **kwargs,
        ...
        if self.encode_y:
            y_new = np.array(
                [-np.sqrt(p1 / p0) if yy == 0 else np.sqrt(p0 / p1) for yy in y_ind]
            )
        else:
            y_new = y_ind
        ...
        if self.threshold is None:
            ...
            raise ValueError(
                "Expected None or a numeric value for " "parameter 'threshold'."
            )
    ...
    def predict(  # noqa: D102
        ...
        y_hat = np.array(
            [
                self.classes_[0] if yy < 0.0 else self.classes_[1]
