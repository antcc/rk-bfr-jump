...
class FPLSBasis(Basis):
    ...
    def _evaluate(self, eval_points: np.ndarray) -> np.ndarray:
        ...
        if isinstance(self.X, FDataGrid):
            ...
            grid = np.linspace(1.0 / 100, 1, 100)

        eval_points_idx = np.abs(grid - eval_points[:, np.newaxis]).argmin(1)
...
class APLS(
    ...
    def fit(
        self, X: FData, y: np.ndarray, eval_points: Optional[np.ndarray] = None
    ) -> APLS:
    ...
    def transform(
        self, X: FData, eval_points: Optional[np.ndarray] = None
    ) -> np.ndarray:
    ...
    def predict(self, X: FData, eval_points: Optional[np.ndarray] = None) -> np.ndarray:
        check_is_fitted(self)
        X = self._argcheck_X(X, eval_points)
    ...
    def _argcheck_X(self, X, eval_points):
        ...
        if hasattr(self, "grid_") and not np.array_equal(X.grid_points[0], self.grid_):
